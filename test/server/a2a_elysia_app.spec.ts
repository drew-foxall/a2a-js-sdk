/**
 * Tests for A2AElysiaApp.
 *
 * These tests verify that the Elysia adapter exhibits the same behavior
...
import { A2AElysiaApp } from '../../src/server/elysia/a2a_elysia_app.js';
...
describe('A2AElysiaApp', () => {
  let mockRequestHandler: A2ARequestHandler;
  let app: A2AElysiaApp;
  let handleStub: SinonStub;

  // Helper to make requests to the Elysia app
  async function makeRequest(
    routes: Array<{ method: string; path: string; handler: (ctx: any) => Promise<Response> }>,
    method: string,
    path: string,
    ...
    const route = routes.find(
      (r) => r.method === method.toLowerCase() && matchPath(r.path, normalizedPath)
    );
  ...
  beforeEach(() => {
    ...
    app = new A2AElysiaApp(mockRequestHandler);
  ...
  describe('constructor', () => {
    it('should create an instance with requestHandler', () => {
      const newApp = new A2AElysiaApp(mockRequestHandler);
      assert.instanceOf(newApp, A2AElysiaApp);
  ...
  describe('getRoutes', () => {
    ...
    it('should include agent card route', () => {
      const routes = app.getRoutes();
      const agentCardRoute = routes.find((r) => r.path.includes('agent-card'));
    ...
    it('should include JSON-RPC route', () => {
      const routes = app.getRoutes();
      const jsonRpcRoute = routes.find((r) => r.method === 'post' && r.path === '/');
  ...
  describe('agent card endpoint', () => {
    ...
    it('should return agent card on custom path when agentCardPath is provided', () => {
      ...
      const customApp = new A2AElysiaApp(mockRequestHandler, { agentCardPath: customPath });
      const routes = customApp.getRoutes();

      const agentCardRoute = routes.find((r) => r.path.includes('custom/agent-card'));
      assert.isDefined(agentCardRoute);
    });
  });
  ...
  describe('route configuration', () => {
    it('should mount routes at basePath', () => {
      const baseApp = new A2AElysiaApp(mockRequestHandler);
      const routes = baseApp.getRoutes('/api/v1');

      const agentCardRoute = routes.find((r) => r.path.includes('agent-card'));
      assert.isDefined(agentCardRoute);
      assert.isTrue(agentCardRoute!.path.startsWith('/api/v1'));
    });
  ...
  describe('authentication integration', () => {
    ...
    it('should handle successful authentication with userBuilder', async () => {
      ...
      const authApp = new A2AElysiaApp(mockRequestHandler, { userBuilder });
  ...
  describe('REST API support', () => {
    it('should include REST routes when enableRest is true', () => {
      const restApp = new A2AElysiaApp(mockRequestHandler, { enableRest: true });
      const routes = restApp.getRoutes();

      const taskRoutes = routes.filter((r) => r.path.includes('/tasks'));
      assert.isTrue(taskRoutes.length > 0, 'Should have task routes');
    });

    it('should not include REST routes when enableRest is false', () => {
      const noRestApp = new A2AElysiaApp(mockRequestHandler, { enableRest: false });
